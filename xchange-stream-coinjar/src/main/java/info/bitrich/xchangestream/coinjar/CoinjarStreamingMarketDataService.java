...
class CoinjarStreamingMarketDataService implements StreamingMarketDataService {
  ...
  public Observable<Trade> getTrades(CurrencyPair currencyPair, Object... args) {
    String channelName = CoinjarStreamingAdapters.adaptCurrencyPairToTradesTopic(currencyPair);
    return service
        .subscribeChannel(channelName)
        .doOnError(
            throwable -> {
              logger.warn(
                  "encountered error while subscribing to channel " + channelName, throwable);
            })
        .filter(node -> node.get("ref").isEmpty())
        .filter(node -> node.has("payload"))
        .map(node -> node.get("payload"))
        .filter(node -> node.has("trades"))
        .flatMap(node -> Observable.fromIterable(node.get("trades")))
        .map(
            node -> {
              CoinjarWebsocketTradeEvent tradeEvent =
                  mapper.treeToValue(node, CoinjarWebsocketTradeEvent.class);
              return CoinjarStreamingAdapters.adoptTradeEvent(currencyPair, tradeEvent);
            });
  }
}
